export class Tabs {
  ...
  _setTabStartState(tab, dataHeight, tabElements, tabContentElement, tabControlElements, dataDelay) {
    ...
    const blockHeight =
      dataHeight === 'max' ? this._returnMaxHeight(tabElements) : tabElements[activeIndex].offsetHeight;
  ...
  _setTabElementHeight(element, transition) {
    ...
    if (dataHeight === 'max') {
      ...
      contentElement.style.height = `${
        this._returnScopeChild(parentElement.querySelectorAll('[data-tabs="element"].is-active'), parentElement)
          .offsetHeight
      }px`;
    }
  ...
  _initTab(tab) {
    ...
    const accordionMedia = tab.getAttribute('data-accordion-media')
      ...
      : null;
    const forLoadTabs = tab.querySelectorAll('[data-tabs="element"].for-load');
    forLoadTabs.forEach((el) => {
      el.classList.remove('for-load');
    });
    ...
    if (accordionMedia && !tab.accordionListener) {
      ...
      accordionMedia.addEventListener(
        'change',
        this.accordionBreakpointChecker.bind(this, accordionMedia, tab, tabElements, tabControlElements)
      );
